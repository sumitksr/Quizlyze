...
export async function fetchContent(file, youtubeUrl, textContent, manualTranscript) {
  ...
  if (file) {
    // Prefer client-side parsing to avoid serverless upload limits and bundling issues
    let clientParsed = false;
    if (typeof window !== "undefined") {
      try {
        const { extractTextFromPdfFile } = await import("./parsePdfClient.js");
        ...
        console.warn("Client-side PDF parsing failed, falling back to API:", clientErr);
      }
    }
    ...
    if (!clientParsed) {
      ...
      if (!apiResponse.ok) {
        ...
        const message =
          errorData?.error ||
          errorData?.details ||
          `PDF processing failed: ${apiResponse.status}`;
